        ...
        for g in G:
            for i in range(num_attributes):
                if g[i] == '?':  # Specializing G
                    for val in np.unique(instance):  # Exploring all possible values
                        if val != instance[i]:  # Avoiding negative example values
...
if __name__ == "__main__":  # Fixed the main function condition
    filename = "ds.csv"  # Ensure this CSV file exists with proper format
